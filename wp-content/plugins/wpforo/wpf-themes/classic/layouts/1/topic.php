...
<div class="wpfl-1">
	...
	<?php foreach($topics as $key => $topic) : ?>
		...
		<?php 
			...
			$topic_url = $wpforo->topic->get_topic_url($topic, $forum);
                ...
                <ul>
					...
					<?php if(!empty($posts) && is_array($posts)) : ?>
						<?php foreach($posts as $post) : ?>
		                    ...
		                    <li>
		                        <div class="wpforo-last-post-title"><i class="fa fa-reply fa-rotate-180 fa-0x wpfcl-0"></i> &nbsp; <a href="<?php echo esc_url($wpforo->post->get_post_url($post['postid'])) ?>" title="<?php wpforo_phrase('REPLY:') ?> <?php echo esc_html( wpforo_text($post['body'], 100, false)) ?>"><?php echo (( $post_body = esc_html(wpforo_text($post['body'], 50, FALSE)) ) ? $post_body : esc_html($post['title'])) ?></a></div> 
