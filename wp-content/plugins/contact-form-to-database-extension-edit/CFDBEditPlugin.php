...
class CFDBEditPlugin extends CF7DBPluginLifeCycle {
    ...
    public function addActionsAndFilters() {
        ...
        add_action('cfdb_edit_fnDrawCallbackJsonForSC', array(&$this, 'fnDrawCallbackJsonForSC'), 10, 2);
    ...
    public function enqueue() {
        ...
        wp_enqueue_script('cfdb.edit', plugins_url('js/cfdb.edit.js?e=1.4', __FILE__), array('jquery', 'jeditable'));
    }
    ...
    public function fnDrawCallbackJSON($tableHtmlId) {
        ...
        $cfdbGetColumnValueUrl = admin_url('admin-ajax.php') . '?action=cfdb-getcolvalue';
    ...
    /**
     * @param $tableHtmlId datatable id to make cells editable in
     * @param $editMode string 'cells' to make cells editable but not column headers, 'true' to make both editable
     */
    public function fnDrawCallbackJsonForSC($tableHtmlId, $editMode = 'true') {
        $cfdbEditUrl = admin_url('admin-ajax.php') . '?action=cfdb-edit';
        $cfdbGetValueUrl = admin_url('admin-ajax.php') . '?action=cfdb-getvalue';
        $cfdbColEditUrl = admin_url('admin-ajax.php') . '?action=cfdb-coledit';
        $cfdbGetColumnValueUrl = admin_url('admin-ajax.php') . '?action=cfdb-getcolvalue';
        $loadImg = plugins_url('img/load.gif', __FILE__);
        if ($editMode == 'cells') {
            echo ",  \"fnDrawCallback\" : function() { cfdbEditableCells('$tableHtmlId','$cfdbEditUrl','$cfdbGetValueUrl','$loadImg'); }";
        } else {
            echo ",  \"fnDrawCallback\" : function() { cfdbEditable('$tableHtmlId','$cfdbEditUrl','$cfdbGetValueUrl','$cfdbColEditUrl','$cfdbGetColumnValueUrl','$loadImg'); }";
        }
    }
